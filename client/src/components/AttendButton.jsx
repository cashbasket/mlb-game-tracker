...
class AttendButton extends React.Component {
  render() {
    ...
    return (
      <Fragment>
        {user && <Button variant={this.props.variant} color={this.props.color} className={this.props.className} size={this.props.size ? this.props.size : 'small'} onClick={() => this.props.isAttending === false ? this.props.addAttendance(user.id, gameId) : this.props.deleteAttendance(gameId, user.id)}>
